...
public class EngineSyncRunner implements Runnable {
     ...
     * this window known workers are allowed to age out normally. Tunable via
     * {@code DISCOVERY_FAILURE_GRACE_MS}; default 5 minutes.
     */
    static long discoveryFailureGraceUs =
            Long.parseLong(System.getenv().getOrDefault("DISCOVERY_FAILURE_GRACE_MS", "300000")) * 1000L;

    /** Last successful discovery time per {@code model/role}, surviving the per-round runner. */
    private static final Map<String, Long> LAST_DISCOVERY_SUCCESS_US = new ConcurrentHashMap<>();
    ...
    public void run() {
        ...
        try {
            ...
            LAST_DISCOVERY_SUCCESS_US.put(discoveryKey(), System.nanoTime() / 1000);
            ...
            // Discovery is down: refresh the staleness clock of known workers so they ride out a
            // transient outage, bounded by discoveryFailureGraceUs (past it they age out normally).
            long nowUs = System.nanoTime() / 1000;
            Long lastSuccessUs = LAST_DISCOVERY_SUCCESS_US.get(discoveryKey());
            ...
            if (withinGrace) {
                ...
                logger.error("service discovery failed, keeping previous worker state within grace, model={}, role={}, error:{}",
                        modelName, roleType, e.getMessage());
            } else {
                logger.error("service discovery failed beyond grace ({}ms), letting workers age out, model={}, role={}, error:{}",
                        discoveryFailureGraceUs / 1000, modelName, roleType, e.getMessage());
            }
        } catch (Exception e) {
