...
class StrategySelectTest(unittest.TestCase):
    ...
    def test_ep_gt1_fp4_env_picks_mega_fp4_strict(self):
        with _env(DSV4_USE_MEGA_MOE_FP4="1"), mock.patch.object(
            MegaMoEFP4Strategy, "can_handle", return_value=True
        ):
            self.assertIs(select_strategy(_cfg(ep_size=4)), MegaMoEFP4Strategy)

    def test_ep_gt1_fp4_env_unavailable_raises(self):
        with _env(DSV4_USE_MEGA_MOE_FP4="1"), mock.patch.object(
            MegaMoEFP4Strategy, "can_handle", return_value=False
        ):
            with self.assertRaises(RuntimeError) as cm:
                select_strategy(_cfg(ep_size=4))
        self.assertIn("Forced MoE strategy 'mega_fp4'", str(cm.exception))
        self.assertIn("cannot handle", str(cm.exception))

    def test_ep_gt1_fp4_and_fused_conflict_raises(self):
        with _env(DSV4_USE_MEGA_MOE_FP4="1", DSV4_USE_MEGA_MOE_FUSED="1"):
            with self.assertRaises(RuntimeError) as cm:
                select_strategy(_cfg(ep_size=4))
        self.assertIn("Conflicting Mega MoE variants", str(cm.exception))
