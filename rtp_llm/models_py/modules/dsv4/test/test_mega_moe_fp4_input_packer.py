...
from rtp_llm.models_py.modules.dsv4.moe.input_packer_fp4 import (
    FusedMegaMoeFP4InputPacker,
    TorchMegaMoeFP4InputPacker,
    get_mega_moe_fp4_input_packer,
)
...
def _make_buf(tokens, dim, topk, device):
    return SimpleNamespace(
        x=torch.empty((tokens, dim // 2), dtype=torch.int8, device=device),
        x_sf=torch.empty((tokens, dim // 64), dtype=torch.int32, device=device),
...
class TestMegaMoeFP4InputPacker(unittest.TestCase):
    def test_dispatch(self):
        old = os.environ.pop("DSV4_MEGA_MOE_FP4_INPUT_PACKER", None)
        try:
            self.assertIsInstance(
                get_mega_moe_fp4_input_packer(), FusedMegaMoeFP4InputPacker
            )
        finally:
            if old is not None:
                os.environ["DSV4_MEGA_MOE_FP4_INPUT_PACKER"] = old
        with _env("DSV4_MOE_STRICT_FUSED", "0"), _env(
            "DSV4_MEGA_MOE_FP4_INPUT_PACKER", "torch"
        ):
            self.assertIsInstance(
                get_mega_moe_fp4_input_packer(), TorchMegaMoeFP4InputPacker
            )
    ...
    def test_fused_rejects_unsupported_without_fallback(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "requires CUDA bf16"):
            FusedMegaMoeFP4InputPacker().pack(x, weights, indices, buf, tokens)

    def test_strict_rejects_torch_packer(self):
        with _env("DSV4_MEGA_MOE_FP4_INPUT_PACKER", "torch"):
        ...
        for tokens in (1, 17, 128):
            with self.subTest(tokens=tokens):
                ...
                with _env("DSV4_MOE_STRICT_FUSED", "0"):
                    TorchMegaMoeFP4InputPacker().pack(x, weights, indices, ref, tokens)
                FusedMegaMoeFP4InputPacker().pack(x, weights, indices, got, tokens)
                self.assertTrue(torch.equal(ref.x.cpu(), got.x.cpu()))
    ...
    def test_zero_tokens_noop(self):
        ...
        FusedMegaMoeFP4InputPacker().pack(
