...
class TestCausalConv1dMaxSeqLenBoundary(unittest.TestCase):
    ...
    def test_no_crash_when_read_offset_out_of_bounds(self):
        ...
        # Keep a padding column in the physical row, then slice it away.  The
        # logical width is num_blocks while stride(0) is num_blocks + 1.  A
        # bounds check that incorrectly uses stride(0) will treat the padding
        # column as valid and load huge_block_id.
        block_map_storage = torch.tensor(
            [list(range(num_blocks)) + [huge_block_id]],
            dtype=torch.int32,
            device=device,
        )
        block_map = block_map_storage[:, :num_blocks]
        self.assertEqual(block_map.shape[1], num_blocks)
        self.assertEqual(block_map.stride(0), num_blocks + 1)
