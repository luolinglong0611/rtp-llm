...
std::shared_ptr<LayerCacheBuffer> LayerCacheBufferUtil::convertLayer(KVCacheResource& resource,
    ...
    return convertLayer(resource, batch_id, layer_id, selected_group_id, start_block_idx, block_count, cp_rank, cp_size);
}
...
std::shared_ptr<LayerCacheBuffer> LayerCacheBufferUtil::convertLayer(KVCacheResource& resource,
    ...
    const auto& block_ids = layer_group_block_ids[static_cast<size_t>(layer_id)][static_cast<size_t>(group_id)]->blocks();
