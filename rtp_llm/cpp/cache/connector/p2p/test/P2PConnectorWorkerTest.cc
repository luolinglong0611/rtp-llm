...
private:
    ...
    static int parseLayerId(const std::string& layer_key) {
        ...
        try {
            const auto layer_begin = layer_key.compare(second_last + 1, 1, "g") == 0 && third_last != std::string::npos ?
                                         third_last + 1 :
                                         second_last + 1;
...
protected:
    ...
    void simulateTaskDone(const std::string& base_key, const std::vector<int>& layer_ids, bool all_success = true) {
        for (int layer_id : layer_ids) {
            std::string layer_key = P2PKeyUtil::makePartitionLayerGroupKey(
                base_key, layer_id, /*group_id=*/2, /*partition_id=*/0);
