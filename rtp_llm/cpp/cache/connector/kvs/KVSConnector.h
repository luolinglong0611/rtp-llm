...
public:
    ...
    struct KVSReadObjectPlan {
        std::string                    object_key;
        std::vector<KVSReadBufferSpec> buffers;
    };

    using BlockBufferResolver = std::function<std::vector<BlockInfo>(int layer_id, int group_id, BlockIdxType block_id)>;
    using KVSReadPlanSender  = std::function<bool(const std::vector<KVSReadObjectPlan>& objects,
    ...
    KVSConnector(const CacheConfig&                 cache_config,
                 ...
                 KVSReadPlanSender                 read_plan_sender = nullptr);
    ~KVSConnector() override;

    bool init();

    bool copyCache(const KVSOperationRequestPB& request, KVSOperationResponsePB& response);
...
private:
    ...
    bool sendTpReadPlan(const std::vector<KVSReadObjectPlan>& objects, const std::string& trace_id) const;
...
private:
    ...
    KVSReadPlanSender                           read_plan_sender_;
