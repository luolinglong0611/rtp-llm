...
public:
    ...
    std::optional<v6d::kvs::LeaseHandle> get(const std::vector<std::string>& object_keys,
        ...
        return client_.get(object_keys, trace_id);
    }
...
KVSClientBackend::get(const std::vector<std::string>& object_keys, const std::string& trace_id) {
    ...
    auto lease_handle = client_->get(object_keys, trace_id);
...
std::optional<KVSReadHandle> KVSClientBackend::create(const std::vector<std::string>& object_keys,
                                                       const std::vector<size_t>&      object_sizes,
...
bool KVSClientBackend::store(const KVSReadHandle& handle, const std::vector<KVSObjectBuffer>& src_buffers) {
    std::unordered_map<std::string, v6d::kvs::ObjectHandle> handles;
    {
        std::lock_guard<std::mutex> lock(lease_mutex_);
        auto iter = lease_handles_.find(handle.handle_id);
...
void KVSClientBackend::release(const KVSReadHandle& handle, const std::string& trace_id) {
    ...
    if (ensureInit()) {
        auto status = client_->release(handle.handle_id, trace_id);
        if (!status.ok()) {
...
void KVSClientBackend::discard(const KVSReadHandle& handle, const std::string& trace_id) {
    if (!handle.valid()) {
        return;
    }
    if (ensureInit()) {
